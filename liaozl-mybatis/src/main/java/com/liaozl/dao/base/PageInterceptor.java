 ...
 * @Description: 分页拦截，根据数据库类型添加相应的分页代码
 * @author Robert
 * @date 2014-9-28 下午5:06:41
 * 
 */
...
public class PageInterceptor extends AbstractInterceptor {
	...
	private String getOraclePageSql(Page page, StringBuffer sqlBuffer) {
		...
		sqlBuffer.insert(0, "select u.*, rownum r from (")
				...
				.append(offset + page.getRowNumber());
	...
	private void setTotalRecord(Page page, MappedStatement mappedStatement,
		...
		if (page.isHaveCountSql()) {//如果使用者指定了统计语句，优先使用。
			countSql = mappedStatement.getConfiguration()
					.getMappedStatement(mappedStatement.getId() + "_count")//按约定，使用sqlId + “_count” 作为统计sqlId
